...
- **Tipos de pergunta** (`quiz`, `tf`, `short`, `slider`, `poll`, `scale`, `nps`, `pin`, `wordcloud`, `brainstorm`, `slide`) definidos em `js/storage.js` (`QUESTION_TYPES`, com categorias em `TYPE_CATEGORIES`) e validados independentemente em `server.js` (`sanitizeQuiz`). Só os `SCORED_TYPES` (`quiz`, `tf`, `short`, `slider`) pontuam e entram na base da nota (`scorableTotal`); a aprovação usa % de acertos deles contra `passScore`. `slide` não recebe resposta, tem layouts (`SLIDE_LAYOUTS`) e avança com `next` direto do estado `question`. `pin` exige imagem. Cada questão tem `reactions` (liga/desliga a barra de emojis do participante).
