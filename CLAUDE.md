...
- **Tipos de pergunta** (`quiz`, `tf`, `short`, `poll`, `scale`, `wordcloud`, `slide`) definidos em `js/storage.js` (`QUESTION_TYPES`, com categorias em `TYPE_CATEGORIES`) e validados independentemente em `server.js` (`sanitizeQuiz`). Só os `SCORED_TYPES` (`quiz`, `tf`, `short`) pontuam e entram na base da nota (`scorableTotal`); a aprovação usa % de acertos deles contra `passScore`. `slide` não recebe resposta e avança com `next` direto do estado `question`.
